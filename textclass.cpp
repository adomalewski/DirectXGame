...
bool TextClass::Initialize(ID3D11Device* device, ID3D11DeviceContext* deviceContext, WindowInfo* windowInfo)
{
	bool result;
	...
	*/
	...
	result = m_Font->Initialize(device, m_windowInfo, "Arial.fnt");
	if (!result)
	{
		MessageBox(NULL, "Could not initialize the font object.", "Error", MB_OK);
		return false;
	}
	...
	if (!result)
	{
		MessageBox(NULL, "Could not initialize the font shader object.", "Error", MB_OK);
		return false;
	}
...
bool TextClass::Render(ID3D11DeviceContext* deviceContext)
{
	...
	result = RenderSentence(deviceContext, m_sentence);
	if (!result)
	{
		return false;
	}
